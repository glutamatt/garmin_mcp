...
def register_tools(app):
    ...
    async def update_workout(workout_id: int, workout_data: WorkoutData) -> str:
        ...
        try:
            ...
            # Step 5: Update using library method with garth
            # Note: Garmin API may return empty body on success, so handle that
            url = f"/workout-service/workout/{workout_id}"
            response = garmin_client.garth.put("connectapi", url, json=merged, api=True)

            # Handle response - Garmin often returns empty body on success
            try:
